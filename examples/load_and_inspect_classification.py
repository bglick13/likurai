...
if __name__ == '__main__':
    ...
    bnn.x.set_value(bnn.train_x.astype(floatX))
    bnn.y.set_value(bnn.train_y.astype(floatX))
    with bnn.model:
        draws = pm.sample_ppc(bnn.trace[0], 300)['likelihood']
    ...
    for i, three in enumerate(best_threes):
        fig, ax = plt.subplots()
        plt.imshow(bnn.train_x[three].reshape(28, 28))
